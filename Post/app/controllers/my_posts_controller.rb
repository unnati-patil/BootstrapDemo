class MyPostsController < ApplicationController
  ...
  def post_index
    ...
    @my_posts = MyPost.paginate(:page => params[:page], :per_page => 5)
  ...
  def post_create
    ...
    @url = post_create_my_posts_path
        ...
        respond_to do |format|
          if @my_post.save
            format.html { redirect_to root_path, :notice => 'User was successfully added!!!' }

          else
            format.html { render :action => "post_add" }
  ...
  def post_update
    ...
    respond_to do |format|
      if @my_post.update_attributes(params[:my_post])
        format.html { redirect_to post_show_my_post_path, :notice => 'Post was successfully updated!!!' }

      else
        format.html {  render :action => "post_edit" }
